...
func TestDelete(t *testing.T) {
	...
	prefix := "SNAP"
	retention := "24d1w1m1y"
	destroy, toDestroy := director.Delete(dataset, SnapshotsList, prefix, retention)
	if destroy == false {
...
func TestNewSnapshot(t *testing.T) {
	...
	prefix := "SNAP"
	creation := "0 * * * *"
	take, SnapshotName := director.NewSnapshot(SnapshotsList, creation, prefix)
...
func TestSend(t *testing.T) {
	...
	prefix := "SNAP"
	SyncPolicy := "asap"
	send, SnapshotUUID := director.Send(dataset, SnapshotsList, SyncPolicy, prefix)
